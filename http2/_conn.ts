...
class Connection extends Flow {
  _streamIds: Record<number, Stream> = [];
  _streamPriorities: Record<number, Stream[]> = [];

  _nextStreamId: number | undefined;
  _lastIncomingStream: number | undefined;
  ...
  _initializeSettingsManagement(settings) {
  ...
  _receiveSettings({ flags, settings }) {
  ...
  _sanityCheckMaxFrameSize(value) {
  ...
  _receivePing({ flags, data }) {
    if (flags.ACK) {
      ...
      if (id in this._pings) {
        ...
        this._log.warn({ data: data }, "Unsolicited PING answer.");
      }
    } else {
  ...
  _initializeFlowControl() {
    ...
    this.on("new_stream", function ({ upstream }) {
  ...
  _setInitialStreamWindowSize(size) {
